...
class InputModalWidget extends Widget{
    ...
    public function init(){
        ...
        $listurl = \Yii::$app->urlManager->createUrl([$this->list]);
        $deleteurl = \Yii::$app->urlManager->createUrl([$this->deleteurl]);
        $script =  <<<JS
...
JS;
        ...
        if(strlen($this->values)){
            ...
            foreach ($this->values_data as $vd) {
                $files[] = Files::find()->where(['file_id' => $vd])->one();
            }
            ...
            foreach ($files as $file){
                ...
                {
                    $this->selected_items_sortable[$file->file_id]['content'] .= "<img class='' style='height:50px; width:100px; object-fit: cover;' src='".$file->getImageSrc()."' />";
                }
                if(in_array($file->type,$this->video_ext))
                {
                    $this->selected_items_sortable[$file->file_id]['content'] .= "<video class='' style='height:auto; display:inline;' src='".$file->getImageSrc()."'> </video>";
                }
                $this->selected_items_sortable[$file->file_id]['content'] .= "<span style=''><i class='glyphicon glyphicon-trash delete_item' onclick='document.filemanager{$this->id}.deleteItem(this);'></i> </span>";
    ...
    public function run(){
        ...
        $upload_widget = FileInput::widget([
            ...
            'pluginEvents' => [
                'fileuploaded' => new JsExpression('function(event, files, extra) {
    document.filemanager'.$this->id.'.query();
        ...
        echo <<<HTML
        <a class="btn btn-primary btn-md" data-toggle="modal" data-target="#filemanagermodel{$this->id}" style="margin-bottom:5px; width:100%; color: white !important;">Выбрать</a>
        {$sort_widget}
...
<!--    <ul id="myTab" class="nav nav-tabs">
    ...
    <div id="myTabContent{$this->id}" class="tab-content">
      <div class="tab-pane show in active" id="filemanager_files{$this->id}">
         <div class="row col">
             {$upload_widget}
        ...
        <div class="row col-md-12 col-xs-12 col-lg-12">
            <div class="fileManagerList{$this->id} col">
                    Loading....
            </div>  
