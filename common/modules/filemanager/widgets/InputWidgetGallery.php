                    ...
                    this.selectAction = function(){
                        ...
                        $(self.resultPaste).find(self.selectBtn).on('click',function(){
                              ...
                              if(self.selected.indexOf(id) == -1){
                                  ...
                                  var appendToHtml = '<li data-key="' + id + '" draggable="true" role="option" aria-grabbed="false"><input name="FileTitle['+id+']" class="form-control" style="margin-top: 10px;" maxlength="255" value="'+title+'" />';
                                  if ('{$this->withDescription}') { appendToHtml += '<textarea name="FileDescription['+id+']" class="form-control" style="margin-top: 5px;" rows="5"></textarea>';
                                  }
                                  appendToHtml += '<a class="btn btn-danger delete_item" style="margin-top: 10px; width: 94%;" onclick="document.filemanager{$this->id}.deleteItem(this);">Удалить</a></li>';
                                  
                                  self.selected.push(id);
                                  $(this_data).hide();
                                  $(this_data).closest(self.filemanager_render_item_parent).find(self.unselectBtn).show();
                                  $('#{$this->file_id_css}-sortable').append(appendToHtml);
...
JS;
        ...
        if(count($this->model_db->{$this->relation_name})){
            ...
            foreach ($via_table_elements as $via_element){
                ...
                $title = htmlspecialchars($via_element->title);

                $this->selected_items_sortable[$via_element->file_id]['content'] .= "<input name='FileTitle[{$file->file_id}]' style='margin-top: 10px;' class='form-control' maxlength='255' value=\"{$title}\" />";

                if ($this->withDescription) {
                    $this->selected_items_sortable[$via_element->file_id]['content'] .= "<textarea name='FileDescription[{$file->file_id}]' style='margin-top: 5px;' class='form-control' rows='5'>{$via_element->description}</textarea>";
                }

                $this->selected_items_sortable[$via_element->file_id]['content'] .= "<a class='btn btn-danger delete_item' style='width: 94%; margin-top: 5px; color: white;' onclick='document.filemanager{$this->id}.deleteItem(this);'>Удалить</a>";
    ...
    public function run(){
        ...
        if ($this->withDescription) {
            $addButton = '<a class="btn btn-primary btn-md" data-toggle="modal" data-target="#filemanagermodel' . $this->id . '" style="margin-bottom:5px; width: 300px; height: 400px; font-size: 100px; padding-top: 100px; margin-top: 5px; color: white;">+</a>';
        } else {
            $addButton = '<a class="btn btn-primary btn-md" data-toggle="modal" data-target="#filemanagermodel' . $this->id . '" style="margin-bottom:5px; width: 100%; margin-top: 5px; color: white;">Добавить</a>';

        }

        echo <<<HTML
        {$sort_widget}
<div class="modal" id="filemanagermodel{$this->id}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel{$this->id}" aria-hidden="true">
...
<!--    <ul id="myTab" class="nav nav-tabs">
    ...
    <div id="myTabContent{$this->id}" class="tab-content">
      <div class="tab-pane in active" id="filemanager_files{$this->id}">
         <div class="row col">
             {$upload_widget}
        ...
        <div class="row col-md-12 col-xs-12 col-lg-12">
            <div class="fileManagerList{$this->id} col" style="margin-top: 20px">
                    Loading....
            </div>  
            <div class="row col-md-12 col-xs-12 col-lg-12 col-sm-12">
                <div class="fileManagerLoadMore{$this->id}" style="margin: 0 auto;">
