							...
							$.ajax({
								...
								success: function(response) 
								{
									...
									if(response.isImage){
										...
										item_detail.find('.file-resolution').show().find('span').text(response.resolutions.low.width + ' x ' + response.resolutions.low.height);
...
JS;
		...
		if($this->model_db->{$this->via_relation}){
			$file = Files::findOne($this->model_db->{$this->via_relation});
			if($file === null){ return false;}
			$this->selected[] = $file->file_id;
			$isImage = $file->getIsImage() ? "filemanager-sortable-item-image" : "";

			$this->selected_items_sortable[$file->file_id]['content'] = '<div data-id="' . $file->file_id . '" class="filemanager-sortable-item ' . $isImage . '"><img src="' . $file->getSrc('small') . '"/><i class="filemanager-sortable-item-icon glyphicon glyphicon-file"></i><span class="filemanager-sortable-item-name">' . Html::encode($file->title) . '</span><span class="filemanager-sortable-item-remove"><i class="glyphicon glyphicon-remove"></i></span></div>';
