...
/**
 * Class FileManagerModalAudio
 *
 * @package common\modules\file\widgets
 */
class FileManagerModalAudio extends InputWidget
{
	/**
	 * @var string
	 */
...
JS;
		...
		if(count($this->model_db->{$this->relation_name})){
			...
			foreach ($via_table_elements as $via_element){
				...
				$this->selected_items_sortable[$via_element->file_id]['content'] = '<div data-id="' . $file->file_id . '" class="filemanager-sortable-item ' . $isImage . '"><img src="' . $file->getSrc('small') . '"/><i class="filemanager-sortable-item-icon glyphicon glyphicon-file"></i><span class="filemanager-sortable-item-name">' . Html::encode($file->title) . '</span><span class="filemanager-sortable-item-remove"><i class="glyphicon glyphicon-remove"></i></span></div>';
