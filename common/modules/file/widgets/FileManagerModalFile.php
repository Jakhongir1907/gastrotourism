...
/**
 * Class FileManagerModalFile
 *
 * @package common\modules\file\widgets
 */
class FileManagerModalFile extends InputWidget
{
	/**
	 * @var string
	 */
	 ...
	 * @var string
	 */
	public $upload_url = '/files/uploads';

	/**
	 * @var string
	 */
	public $detail_view_url = '/files/detail';

	/**
	 * @var string
	 */
	public $detail_update_url = '/files/update-detail';

	/**
	 * @var string
	 */
	public $file_delete_url = '/files/delete-file';
				...
				this.checkItem = function(file_id, file_title, file_img, file_isimg) 
				{
					...
					if(self.multiple){
						if(self.selected.includes(file_id)) {  
							...
							$(self.self_block + ' ' + self.sortable).append('<li data-key="' + file_id + '" draggable="true"><div data-id="' + file_id + '" class="filemanager-sortable-item ' + item_image + '"><img src="' + file_img + '"/><i class="filemanager-sortable-item-icon glyphicon glyphicon-file"></i><input name="FileTitle[' + file_id + ']" class="filemanager-sortable-item-name" value="' + file_title + '" /><span class="filemanager-sortable-item-remove"><i class="glyphicon glyphicon-remove"></i></span></div></li>');
...
JS;
		...
		if(count($this->model_db->{$this->relation_name})){
			...
			foreach ($via_table_elements as $via_element){
				...
				$this->selected_items_sortable[$via_element->file_id]['content'] = '<div data-id="' . $file->file_id . '" class="filemanager-sortable-item ' . $isImage . '"><img src="' . $file->getSrc('small') . '"/><i class="filemanager-sortable-item-icon glyphicon glyphicon-file"></i><input name="FileTitle[' . $file->file_id . ']" class="filemanager-sortable-item-name" value="' . Html::encode($via_element->title) . '" /><span class="filemanager-sortable-item-remove"><i class="glyphicon glyphicon-remove"></i></span></div>';
...
//		if($this->mime_type && in_array($this->mime_type, self::$_allowed_types))
//            $searchModel->extension = ['mp3', 'wav', 'ogg'];
		...
		$model->query->orderBy(['file_id' => SORT_DESC]);

        return $this->render('FileManagerModalAudio', [
			'model'     => $model,
			'widget'    => $this,
			'form'      => $this->form,
