...
class FileManagerEditorModal extends InputWidget
{
	 ...
	 * @var string
	 */
	public $upload_url = '/files/upload';
		...
		$script =  <<<JS
			...
			var filemanager = function(editor)
			{
							...
							$.ajax({
								...
								success: function(response) 
								{
									...
									if(response.isImage){
										...
										file_resolution.show().find('span').text(response.resolutions.low.width + ' × ' + response.resolutions.low.height);
									...
									$.map(response.resolutions, function(item) {
										file_append_size.append('<option value="' + item.src + '">' + item.title + ' - ' + item.width + ' × ' + item.height + '</option>');
									});
									
									if(response.resolutions.low){
									    file_append_size.val(response.resolutions.low.src);
									}
				...
				this.addToInput = function() 
				{
					...
					console.log(sidebar);
					console.log(sidebar.find('.file-append-size select'));
...
//		if(!Yii::$app->user->can(User::ROLE_EDITOR)){
