...
class CategoryModelBehavior extends Behavior
{
     ...
     * @return string
     */
    public function getCategoryPrint($class = ""){
     ...
     * @return string
     */
    public function getCategoriesPrintTag($tag = "a",$limit = null,$class = ""){
        $data = "";
        ...
        $i = 0;
            foreach ($categories as $category){
                ...
                $i++;
                $data .= '<'.$tag.' href="'.$category['link'].'" class="'.$class.'">';
                $data .= $category['name'];
                $data .= "</".$tag.">";
                if($limit == $i){break;}
            }
     ...
     * @param string $tag
     */
    public function getCategoryPrintTag($tag = "a",$class = ""){
       return $this->getCategoriesPrintTag($tag,1,$class);
    }
